...
### Fixed
- **Veeam SOSAPI: `system.xml` `ProtocolVersion` and `ModelName` reverted to quoted format** — Veeam's SOSAPI parser requires the values as JSON-style quoted strings inside the XML element (`` `"1.0"` `` and `` `"MaxIOFS"` ``), consistent with how MinIO implements the same spec. A previous change had incorrectly replaced these with plain unquoted strings, breaking SOSAPI recognition and capacity reporting. (`pkg/s3compat/sosapi.go`)
