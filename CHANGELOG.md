...
- **Download folder as ZIP button in bucket browser** — folder rows in the object table now show a `FolderDown` icon button that triggers the ZIP download for that folder. The button is hidden for global admins accessing tenant buckets (read-only restriction). (`web/frontend/src/pages/buckets/[bucket]/index.tsx`, `web/frontend/src/lib/api.ts`)
