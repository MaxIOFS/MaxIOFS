...
type StorageConfig struct {
	...
	// Seeds the KEK on the first start of an empty deployment; ignored after
	// that, the key lives in the database.
...
type AuthConfig struct {
	...
	// Encrypts stored secrets: identity providers, replication targets, shares.
	// It is not an S3 credential, whatever its previous name suggested.
...
func Load(cmd *cobra.Command) (*Config, error) {
	...
	if cfg.Auth.EncryptionSecret == "" {
		if legacy := v.GetString("auth.secret_key"); legacy != "" {
			cfg.Auth.EncryptionSecret = legacy
			logrus.Warn("auth.secret_key is now auth.encryption_secret; rename it, and keep the same value or what it encrypted stops being readable")
		}
	}
