...
func TestAuthConfig_Struct(t *testing.T) {
	cfg := AuthConfig{
		EnableAuth:       true,
		JWTSecret:        "secret",
		EncryptionSecret: "wJalrXUtnFEMI/K7MDENG/bPxRfiCYEXAMPLEKEY",
	}

	assert.True(t, cfg.EnableAuth)
	assert.Equal(t, "secret", cfg.JWTSecret)
	assert.Equal(t, "wJalrXUtnFEMI/K7MDENG/bPxRfiCYEXAMPLEKEY", cfg.EncryptionSecret)
}
...
func TestNormalizeURLForCompare(t *testing.T) {
	cases := []struct {
		...
		{"https://storage.example.com", "https://storage.example.com", true}, // identical
		{"http://x:8080", "http://x:8080/", true},                            // trailing slash
		{"https://X.com", "https://x.com", true},                             // case-insensitive host
		{"https://x.com/s3", "https://x.com", false},                         // distinct path prefix
		{"https://x.com:8080", "https://x.com:8081", false},                  // distinct port
		{"https://s3.example.com", "https://console.example.com", false},     // distinct host
	}
	for _, c := range cases {
		got := normalizeURLForCompare(c.a) == normalizeURLForCompare(c.b)
		if got != c.equal {
			t.Errorf("normalize equality %q vs %q: got %v want %v", c.a, c.b, got, c.equal)
		}
	}
}
